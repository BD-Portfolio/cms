...
export default function Home() {
    ...
    const { pageSelected, createPage, updatePage } = useContext(PageContext);
    const { activePlugins } = useContext(PostContext);
    ...
    const submit = async (event: any) => {
        ...
        if (result) {
            setMsg({ value: "Failed to create page", type: "error" });
        } else {
            setMsg({ value: "Page create successfully", type: "success" });
        }
    }

    useEffect(() => {
        if (pageSelected) {
            setTitle(pageSelected.title);
            setContent(pageSelected.content);
            setHashtag(pageSelected.hashtag ?? "");
            setSlug(pageSelected.slug);
        }
    }, [])
